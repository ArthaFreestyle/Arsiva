...
- **Secret storage:** Redis keys `otp:verify:{email}` (6-digit OTP) / `otp:reset:{email}` (32-byte URL-safe token from `utils.GenerateResetToken`) store the **SHA-256 hash** of the secret (never plaintext) plus an `attempts` counter, with a TTL. Both share the `storeSecret` helper. A resend cooldown key `otp:cooldown:{purpose}:{email}` throttles re-issues. Secrets are single-use (deleted on success) and invalidated after `otp_max_attempts` wrong tries. All tunables live under the `email` block in `config.json` (`otp_ttl_minutes` — also the reset-link TTL, `otp_max_attempts`, `otp_resend_cooldown_seconds`, `reset_password_url`).
