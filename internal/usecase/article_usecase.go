...
type ArticleUseCase interface {
	...
	GetAllArticleManage(ctx context.Context, page int, size int, search string, userId string, role string) ([]*model.ArticleResponse, int, error)
	GetArticleByIdManage(ctx context.Context, articleId string, userId string, role string) (*model.ArticleResponse, error)
	UpdateArticleManage(ctx context.Context, article *model.ArticleUpdateRequest, articleId string, userId string, role string) (*model.ArticleResponse, error)
	DeleteArticleManage(ctx context.Context, articleId string, userId string, role string) error
}
...
func (u *articleUseCaseImpl) GetAllArticleManage(ctx context.Context, page int, size int, search string, userId string, role string) ([]*model.ArticleResponse, int, error) {
	if page < 1 {
		page = 1
	}
	if size < 1 || size > 50 {
	...
	articles, total, err := u.ArticleRepository.GetAllArticleManage(ctx, page, size, search, userId, role)
	if err != nil {
		u.Log.Warnf("error when get all article manage: %v", err)
		return nil, 0, fiber.ErrInternalServerError
	}
...
func (u *articleUseCaseImpl) GetArticleByIdManage(ctx context.Context, articleId string, userId string, role string) (*model.ArticleResponse, error) {
	article, err := u.ArticleRepository.GetArticleByIdManage(ctx, articleId, userId, role)
	if err != nil {
		u.Log.Warnf("error when get article by id manage: %v", err)
		return nil, fiber.ErrNotFound
	}
...
func (u *articleUseCaseImpl) UpdateArticleManage(ctx context.Context, article *model.ArticleUpdateRequest, articleId string, userId string, role string) (*model.ArticleResponse, error) {
	err := u.Validator.Struct(article)
	if err != nil {
		u.Log.Warnf("error when validate article: %v", err)
		return nil, fiber.ErrBadRequest
	...
	_, err = u.ArticleRepository.GetArticleByIdManage(ctx, articleId, userId, role)
	if err != nil {
		u.Log.Warnf("article not found or access denied: %v", err)
...
func (u *articleUseCaseImpl) DeleteArticleManage(ctx context.Context, articleId string, userId string, role string) error {
	_, err := u.ArticleRepository.GetArticleByIdManage(ctx, articleId, userId, role)
	if err != nil {
		u.Log.Warnf("article not found or access denied: %v", err)
